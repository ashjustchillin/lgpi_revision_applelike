...
export default function DataIO({ notes, mods, onImport, onZendesk }) {
  ...
  const handleFileChange = async e => {
    ...
    try {
      ...
      setImportResult({ ok: true, msg: `${data.notes.length} fiches et ${data.mods.length} modules prets`, data })
    } catch (err) {
        ...
        {open && (
          ...
          >
            ...
            <div className="mb-3 pb-3 border-b border-gray-100 dark:border-zinc-700">
              ...
              >
                {importing ? 'Lecture...' : 'Choisir un fichier JSON'}
              </motion.button>
              <AnimatePresence>
                {importResult && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
                    <div className={`mt-2 p-2.5 rounded-xl text-xs ${importResult.ok ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-500'}`}>
            ...
            {/* Zendesk - redirige vers la page dédiée */}
            {onZendesk && (
              <div>
                ...
                <motion.button whileTap={{ scale: .96 }}
                  onClick={() => { setOpen(false); onZendesk() }}
                  className="w-full py-2 border-2 border-dashed rounded-xl text-sm font-semibold transition-all flex items-center justify-center gap-2"
                  style={{ borderColor: 'var(--accent)', color: 'var(--accent)', background: 'var(--accent-bg)' }}
                >
                  🎫 Ouvrir l'import Zendesk
                </motion.button>
              </div>
            )}
          </motion.div>
