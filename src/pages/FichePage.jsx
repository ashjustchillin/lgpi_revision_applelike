...
export default function FichePage({ note, mod, allNotes, onBack, onEdit, onDelete, onFiche, onToast, isPinned, onTogglePin, masteryLevel, onMasteryChange, isAdmin, account, onCopyLink }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
      ...
      <div className="flex items-start justify-between gap-3 mb-2 flex-wrap">
        <motion.h1 layoutId={"title-" + note.id} className="text-2xl font-bold tracking-tight leading-tight flex-1" style={{ color: 'var(--text-1)', letterSpacing: '-.04em' }}>{note.title}</motion.h1>
        <motion.div layoutId={"badge-" + note.id}><TypeBadge typeId={note.type} size="lg" /></motion.div>
      </div>

      {pathSteps.length > 0 && (
        <div className="inline-flex items-center gap-1.5 flex-wrap mb-4 px-3 py-2 rounded-xl border" style={{ background: 'var(--surface)', borderColor: 'var(--border)' }}>
          {pathSteps.map((s, i) => (
      ...
      {questions && questions.length > 0 && questions[0]?.r && (
        <div className="mb-4 p-4 rounded-2xl border" style={{ background: '#f0f9ff', borderColor: '#7dd3fc' }}>
          ...
          <div>
            {showAnswer ? (
              <div>
                <p className="text-sm p-3 rounded-xl mb-3" style={{ background: '#f0fdf4', color: '#166534' }}>✓ {questions[currentQ]?.r}</p>
                <div className="flex gap-2">
                  {currentQ < questions.length - 1
                    ? <button onClick={() => { setCurrentQ(q => q + 1); setShowAnswer(false) }} className="btn-accent text-xs">Question suivante →</button>
                    : <button onClick={() => { setCurrentQ(0); setShowAnswer(false) }} className="btn-accent text-xs">🔄 Recommencer</button>}
                </div>
              </div>
            ) : (
              <button onClick={() => setShowAnswer(true)} className="text-xs px-4 py-2 rounded-xl font-semibold" style={{ background: '#f0f9ff', color: '#0369a1', border: '1px solid #7dd3fc' }}>Voir la réponse</button>
            )}
          </div>
        </div>
      )}
      ...
      <div className="flex items-center gap-2 flex-wrap mb-4">
        ...
        <button onClick={handleQuestions} disabled={questionsLoading} className="flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-xl border transition-all" style={{ background: 'var(--surface)', borderColor: 'var(--border)', color: 'var(--text-2)' }}>{questionsLoading ? '⏳' : questions ? '✕ Questions' : '❓ Questions IA'}</button>
        <button onClick={handleSuggestLinks} disabled={linksLoading} className="flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-xl border transition-all" style={{ background: 'var(--surface)', borderColor: 'var(--border)', color: 'var(--text-2)' }}>{linksLoading ? '⏳' : suggestedLinks ? '✕ Suggestions' : '🔗 Fiches liees IA'}</button>
        <button onClick={() => setFocusMode(true)} className="flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-xl border transition-all" style={{ background: 'var(--surface)', borderColor: 'var(--border)', color: 'var(--text-2)' }}>🎯 Focus</button>
        <button onClick={onTogglePin} className="flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-xl border transition-all"
      ...
      {/* Niveau de maitrise */}
      <div className="mb-4">
        <p className="text-[11px] font-semibold uppercase tracking-wider mb-2" style={{ color: 'var(--text-3)' }}>Niveau de maitrise</p>
      ...
      {(note.images || []).length > 0 && (
        <div className="mb-4 pt-4">
          <p className="text-[11px] font-semibold uppercase tracking-wider mb-3" style={{ color: 'var(--text-3)' }}>📷 Captures d'écran</p>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {note.images.map((img, idx) => (
              <div key={idx} onClick={() => setExpandedImage(img)}
                className="rounded-xl overflow-hidden border hover:shadow-md transition-shadow cursor-pointer"
                style={{ borderColor: 'var(--border)' }}>
                <img src={img.url} alt={img.name || 'Capture'} className="w-full h-auto" loading="lazy" />
              </div>
            ))}
          </div>
        </div>
        ...
        {expandedImage && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <button onClick={() => setExpandedImage(null)}
              className="absolute top-6 right-6 w-10 h-10 rounded-full bg-white/20 text-white flex items-center justify-center text-xl">✕</button>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        <div className="mb-4 p-4 rounded-2xl border" style={{ background: 'var(--surface-2)', borderColor: 'var(--border)' }}>
          ...
          <div className="space-y-2">
            {suggestedFiches.map(f => (
              <motion.button key={f.id} whileHover={{ x: 3 }} whileTap={{ scale: .98 }} onClick={() => onFiche(f.id)}
                className="w-full flex items-center gap-3 p-3 rounded-xl text-left" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
            ...
            {questions.map((q, i) => (
              <div key={i} className="card-base p-4">
                ...
                {questionsRevealed[i] ? (
                  ...
                  <button onClick={() => setQuestionsRevealed(r => ({ ...r, [i]: true }))} className="text-xs px-3 py-1.5 rounded-lg font-medium" style={{ background: 'var(--surface-2)', color: 'var(--text-2)' }}>Voir la reponse</button>
                )}
              </div>
            ))}
          </div>
      ...
      {linkedNotes.length > 0 && (
        <div className="mb-4">
          <p className="text-[11px] font-semibold uppercase tracking-wider mb-2" style={{ color: 'var(--text-3)' }}>Fiches liees</p>
          <div className="flex flex-wrap gap-2">
            {linkedNotes.map(ln => (
              <motion.button key={ln.id} whileHover={{ y: -1 }} whileTap={{ scale: .97 }} onClick={() => onFiche(ln.id)}
                className="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl text-sm font-medium border transition-all"
                style={{ background: 'var(--surface)', borderColor: 'var(--border)', color: 'var(--text-2)' }}>{ln.title}</motion.button>
            ))}
          </div>
        </div>
      )}
      ...
      {(note.tags || []).length > 0 && (
        <div className="flex flex-wrap gap-1.5 mb-3">
          {note.tags.map(t => (<span key={t} className="text-xs px-2.5 py-1 rounded-full font-medium" style={{ background: mod.bg, color: mod.tc }}>{t}</span>))}
        </div>
      )}
      ...
      {isAdmin && (
        <div className="flex gap-3 flex-wrap mb-6">
          <motion.button whileTap={{ scale: .96 }} onClick={onEdit} className="px-5 py-2.5 text-white text-sm font-medium rounded-full hover:brightness-90" style={{ background: mod.color }}>Modifier</motion.button>
          <motion.button whileTap={{ scale: .96 }} onClick={() => setConfirm(true)} className="px-5 py-2.5 text-sm font-medium rounded-full border" style={{ color: '#ef4444', borderColor: '#fecaca', background: 'transparent' }}>Supprimer</motion.button>
        </div>
      )}
      ...
      {focusMode && (
        <div className="fixed inset-4 z-50 rounded-3xl overflow-auto p-6" style={{ background: 'var(--surface)', boxShadow: 'var(--shadow-xl)' }}>
          <button onClick={() => setFocusMode(false)} className="absolute top-4 right-4 text-sm px-3 py-1.5 rounded-full" style={{ background: 'var(--surface-2)', color: 'var(--text-2)' }}>✕ Quitter le focus</button>
      ...
      {confirm && (
        <ConfirmModal title="Supprimer cette fiche ?" message={'"' + note.title + '" sera definitivement supprimee.'}
