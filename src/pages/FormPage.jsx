...
export default function FormPage({ note, mods, notes, curMod, onSave, onCancel }) {
  ...
  const handleSave = async () => {
    ...
    const data = {
      ...
      type, links, tags, images, date: todayStr(),
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}>
      ...
      {showTemplates && !note && (
        <div className="mb-5">
          <p className="text-xs font-semibold uppercase tracking-wider mb-2" style={{ color: 'var(--text-3)' }}>Choisir un template</p>
      ...
      {doublons.length > 0 && (
        <div className="mb-4 p-3 rounded-xl border" style={{ background: '#fef9c3', borderColor: '#fbbf24' }}>
          <p className="text-xs font-semibold mb-1" style={{ color: '#92400e' }}>⚠️ {doublons.length} fiche{doublons.length > 1 ? 's' : ''} similaire{doublons.length > 1 ? 's' : ''} detectee{doublons.length > 1 ? 's' : ''}</p>
          {doublons.map(d => (<p key={d.id} className="text-xs" style={{ color: '#78350f' }}>• {d.title}</p>))}
          <button type="button" onClick={() => setDoublons([])} className="text-[10px] mt-1 underline" style={{ color: '#92400e' }}>Ignorer</button>
        </div>
      )}
      ...
      <div className="bg-white dark:bg-zinc-800 rounded-2xl border border-gray-200 dark:border-zinc-700 p-6 max-w-2xl">
        ...
        <div className="mb-6 pb-6 border-b border-gray-100 dark:border-zinc-700">
          ...
          <div className="mb-3">
            <div className="flex items-center justify-between mb-1.5">
              ...
              {content.trim() && (
                <motion.button whileTap={{ scale: .95 }} onClick={handleSuggererTitre} disabled={aiLoading === 'titre'}
                  ...
                  {aiLoading === 'titre' ? <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 border border-purple-400 border-t-transparent rounded-full animate-spin inline-block" /> Génération...</span> : '✨ Suggérer un titre'}
                </motion.button>
              )}
            </div>
            <input value={title} onChange={e => setTitle(e.target.value)} placeholder="Titre de la fiche" className="input-base" />
            <AnimatePresence>
                ...
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden mt-2">
                  <div className="p-3 bg-purple-50 dark:bg-purple-950 border border-purple-200 dark:border-purple-800 rounded-xl">
                    ...
                    <div className="flex gap-2">
                      <button onClick={acceptPreview} className="text-xs font-semibold text-white bg-purple-500 px-3 py-1 rounded-lg">Utiliser</button>
                      <button onClick={() => setAiPreview(null)} className="text-xs text-purple-400">Ignorer</button>
                    </div>
                  </div>
                </motion.div>
              )}
            ...
            <div className="flex gap-2 flex-wrap">
              {TYPES.map(tp => (
                <motion.button key={tp.id} whileTap={{ scale: .95 }} onClick={() => setType(tp.id)}
                  ...
                  style={type === tp.id ? { background: tp.bg, borderColor: tp.color, color: tp.color, fontWeight: 600, border: '1.5px solid' } : { borderColor: '#d1d5db', color: '#9ca3af', background: 'transparent', border: '1.5px solid' }}>
                  {tp.emoji} {tp.label}
                </motion.button>
              ))}
            </div>
        ...
        <div className="mb-6 pb-6 border-b border-gray-100 dark:border-zinc-700">
          ...
          <div className="mb-3">
            ...
            <input value={path} onChange={e => setPath(e.target.value)} placeholder="ex: Stock > Inventaires > Nouvel inventaire" className="input-base" />
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              {content.trim() && (
                <motion.button whileTap={{ scale: .95 }} onClick={handleReformuler} disabled={!!aiLoading}
                  ...
                  {aiLoading === 'reformuler' ? <span className="flex items-center gap-1.5"><span className="w-3 h-3 border-2 border-purple-400 border-t-transparent rounded-full animate-spin" />Reformulation...</span> : '✨ Reformuler avec IA'}
                </motion.button>
              )}
            </div>
            <div className="flex items-center justify-between mb-1.5">
            ...
            {showPreview ? (
              ...
              <textarea value={content} onChange={e => setContent(e.target.value)}
                ...
                rows={8} className="input-base resize-y leading-relaxed" />
            )}
              ...
              {aiPreview?.type === 'reformuler' && (
                <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                  ...
                  <div className="bg-purple-50 dark:bg-purple-950 px-4 py-2.5 flex items-center justify-between border-b border-purple-200 dark:border-purple-800">
                    ...
                    <div className="flex gap-2">
                      <button onClick={acceptPreview} className="text-xs font-semibold text-white bg-purple-500 px-3 py-1 rounded-lg">Utiliser cette version</button>
                      <button onClick={() => setAiPreview(null)} className="text-xs text-purple-400 px-2">Ignorer</button>
                    </div>
                  </div>
          ...
          <div className="mt-4">
            <ImageUploader
              noteId={note?.id || 'new-' + Date.now()}
              images={images}
              onImagesChange={setImages}
              onInsertMarkdown={(md) => setContent(c => c + md)}
              isAdmin={true}
            />
          </div>
        </div>
        ...
        <div className="mb-6">
          ...
          <div className="mb-4">
            ...
            <div className="border border-gray-200 dark:border-zinc-700 rounded-xl overflow-hidden">
              <input value={linkSearch} onChange={e => setLinkSearch(e.target.value)} placeholder="Filtrer les fiches..."
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              {(content.trim() || title.trim()) && (
                <motion.button whileTap={{ scale: .95 }} onClick={handleSuggererTags} disabled={aiLoading === 'tags'}
                  className="flex items-center gap-1 text-[10px] font-medium text-purple-500 hover:text-purple-600 disabled:opacity-50 transition-colors">
                  {aiLoading === 'tags' ? <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 border border-purple-400 border-t-transparent rounded-full animate-spin inline-block" /> Analyse...</span> : '✨ Suggérer des tags'}
                </motion.button>
              )}
            </div>
            <AnimatePresence>
                ...
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden mb-2">
                  <div className="p-3 bg-purple-50 dark:bg-purple-950 border border-purple-200 dark:border-purple-800 rounded-xl">
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-2">
                      {aiPreview.value.map(t => (<span key={t} className="text-xs px-2.5 py-1 rounded-full font-medium bg-purple-100 dark:bg-purple-900 text-purple-600 dark:text-purple-300">{t}</span>))}
                    </div>
                    <div className="flex gap-2">
                      <button onClick={acceptPreview} className="text-xs font-semibold text-white bg-purple-500 px-3 py-1 rounded-lg">Ajouter tous</button>
                      <button onClick={() => setAiPreview(null)} className="text-xs text-purple-400">Ignorer</button>
                    </div>
                  </div>
                </motion.div>
            ...
            <div className="relative" ref={tagRef}>
              <div className="flex flex-wrap gap-1.5 p-2 border border-gray-200 dark:border-zinc-700 rounded-xl min-h-10 cursor-text bg-gray-50 dark:bg-zinc-900"
                ...
                <input value={tagInput}
                  ...
                  onKeyDown={e => { if (e.key === 'Enter' || e.key === ',') { e.preventDefault(); addTag(tagInput) }; if (e.key === 'Escape') setSuggestions([]) }}
                ...
                <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  {suggestions.map(s => (
                    <button key={s} onMouseDown={e => { e.preventDefault(); addTag(s) }}
                      className="block w-full text-left px-3 py-2 text-sm hover:bg-gray-50 dark:hover:bg-zinc-700 text-gray-700 dark:text-zinc-300">{s}</button>
                  ))}
                </motion.div>
              )}
            </div>
